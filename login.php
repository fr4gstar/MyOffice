<?php
  session_start();
  if (isset($_SESSION['sid']) == false AND $_SESSION['login'] == false){
    $_SESSION['sid'] = uniqid();
  ...
  if($_SESSION['login_counter'] >= 5){
    ...
    if(isset($_GET['email']) AND isset($_GET['password']) AND (strlen($_GET['email']) > 0) AND (strlen($_GET['password']) > 0)) {
    ...
    if(!$conn)
      ...
      if(mysql_num_rows($res) == 1){
        ...
        echo "PHP: Email oder Passwort stimmen nicht!" .$_SESSION['login_counter']. ". Login Versuch";      
      }
    }else{
