jQuery(document).ready(function(){
  ...
  jQuery("#overview").append("<div id='ov_header'>Zuletzt hinzugef&uuml;gt ...</div><div id='ov_contacts' class='overview_line'>Kontakte</div><div id='ov_notes' class='overview_line'>Notizen</div><div id='ov_events' class='overview_line'>Termine</div><div id='ov_tasks' class='overview_line'>Aufgaben</div>");
  jQuery("#ov_contacts").append("<table id='ov_contactTable' href='#'><tr><th></th><th>Vorname</th><th>Nachname</th></tr></table>");
  /*
  function createContacts(data){
      ...
      while(json[counter]){
        ...
        tableContentHTML += "<tr><td><button id='btn_viewContact"+contactID+"' class='opener' data-id='#dialog"+contactID+"'>Bearbeiten</button></td><td>"+firstname+"</td><td>"+lastname+"</td></tr>";
        counter++;
      };
  ...
  function updateContacts(){
    jQuery.get("contacts.php",{},function(data){
      ...
      jQuery("#showContacts").append(
        "<table id='contactTable' href='#'><tr><th></th><th>Vorname</th><th>Nachname</th><th>Firma</th></tr></table>"
      );
      var json = $.parseJSON(data);
      var counter = 1;
      while(json[counter]){
        jQuery("#contactTable").append(
          "<tr><td><button id='btn_viewContact"+json[counter][counter][0]+"'>*</button><button id='btn_deleteContact"+json[counter][counter][0]+"'>-</button><button id='btn_editContact"+json[counter][counter][0]+"'>/</button></td><td>"+json[counter][counter][1]+"</td><td>"+json[counter][counter][2]+"</td><td>"+json[counter][counter][3]+"</td></tr>"
        );    
        counter++;
      }
